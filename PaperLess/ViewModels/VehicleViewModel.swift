...
class VehicleViewModel: ObservableObject {
    ...
    @Published var vehicles: [Vehicle] = [
        Vehicle(nfcTag: NFCTag(id: UUID(), tagID: "001", name: "Porsche 911", status: DeviceStatus.available, icon: ObjectIcon.car.rawValue), brand: "Porsche", kennzeichen: "TÜ-TS-001", color: CarColor.blue, kilometerstand: 10000, serviceInterval: ServiceInterval.nine, lastMaintenance: Date.now),
        Vehicle(
                nfcTag: NFCTag(
    ...
    private static func makeEmptyVehicle() -> Vehicle {
        Vehicle(
            nfcTag: NFCTag(
                id: UUID(),
                tagID: "",
    ...
    init() {
        self.newVehicle = Self.makeEmptyVehicle()
    }
    ...
    let kmFormatter: NumberFormatter = {
        ...
        fmt.usesGroupingSeparator = true    // Tausender-Punkt aktivieren
        fmt.isLenient             = true    // lockeres Parsen erlauben
        return fmt
    }()
    
    func addVehicle() {
        vehicles.append(newVehicle)
        newVehicle = Self.makeEmptyVehicle()
    }
