...
struct NfcView: View {
    @State private var showAddNfcTag: Bool = false
    ...
    var body: some View {
        NavigationStack {
            List {
                SortPicker(selectedOption: $sortOption)
                
                ForEach(gefiltertUndSortiert(), id: \.id) { dokument in
                    NavigationLink(destination: NfcDetailView(nfcDocument: binding(for: dokument))) {
                        NfcDocumentRow(nfcDocument: dokument)
                    }
                }
                .listStyle(.plain)
            }
    ...
    //FUNKTIONEN
    private func sortierFunktion(for option: SortOption) -> (NfcDocument, NfcDocument) -> Bool {
            switch option {
            case .name:
                return { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
            case .status:
                let reihenfolge: [DeviceStatus] = [.available, .loaned, .defect, .lost]
                return { a, b in
                    let iA = reihenfolge.firstIndex(of: a.status) ?? 0
                    let iB = reihenfolge.firstIndex(of: b.status) ?? 0
                    return iA != iB ? (iA < iB) : (a.name < b.name)
                }
            case .category:
                let reihenfolgeKategorie: [Category] = [.kleingerät, .werkzeug, .maschine, .fahrzeug]
                return { a, b in
                    let iA = reihenfolgeKategorie.firstIndex(of: a.category ?? .kleingerät) ?? 0
                    let iB = reihenfolgeKategorie.firstIndex(of: b.category ?? .kleingerät) ?? 0
                    return iA != iB ? (iA < iB) : (a.name < b.name)
                }
            }
        }
    
    private func gefiltertUndSortiert() -> [NfcDocument] {
            let gefiltert = nfcDocuments.filter { doc in
                searchText.isEmpty
            ...
            return gefiltert.sorted(by: sortierFunktion(for: sortOption))
        }
    
    private func binding(for document: NfcDocument) -> Binding<NfcDocument> {
        guard let index = nfcDocuments.firstIndex(where: { $0.id == document.id }) else {
            fatalError("Dokument nicht gefunden")
        }
        return $nfcDocuments[index]
    }
}
